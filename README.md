...
Tools for Symfony Messenger including Symfony Bundle for easy configuration.
...
return [
  // ...
  Sokil\MessageBus\MessageBusBundle::class => ['all' => true],
];
```
...
This is done in service `\Sokil\MessageBus\Service\TypeLocator`:
...
If you use Symfony framework, this already done in service `message_bus.type_locator`.
...
This creates service `message_bus.serializer.some_serializer.json`.
...
With `\Sokil\MessageBus\Middleware\AmqpMessageRoutingKeyByTypeMiddleware` we may generate routing key 
...
When used in Symfony framework, service `message_bus.middleware.amqp_message_routing_key_by_type` 
